...
import {Message, Content} from "./model/websocket"
...
export default class WebSocketServer{
    ...
    private handleConn(socket: ws): void {
        ...
        var welcomeRelay: Content = ServerMessage("Welcome! There are currently " + this.wssChat.clients.size + " clients connected. Please refrain from spam & immature activity.")
        socket.send(JSON.stringify(welcomeRelay));
        /* To all but new Client on Connection */
        var connectedRelay: Content = ServerMessage("New client has connected, with current total of " + this.wssChat.clients.size + " clients.")
